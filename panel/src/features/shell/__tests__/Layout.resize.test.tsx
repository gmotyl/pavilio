...
describe("resizing the shell's sidebars", () => {
  ...
  it("an expanded sidebar renders a resize handle on desktop", () => {
    ...
    // The applied width now comes from the hook, in pixels. `--sidebar-width`
    // stays in `index.css` as the documented default and is read by nothing:
    // a `var()` here would be a width the hook cannot start a drag from, since
    // it takes its starting point from React state and never a measurement.
