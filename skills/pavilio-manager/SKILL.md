...
- **Merged but unarchived** — when an active change dir (un-archived under `openspec/changes/`, see [[pavilio-openspec-storage]] for backend resolution) references a PR in its `proposal.md`, check its state within the PR-state budget row above (max 3/run, oldest first; use the right token for the repo). Merged PR + change still un-archived → suggest `/pavilio-archive-plan <project> <change-id>` under Risks ("shipped, not archived"). Changes beyond the cap: note "PR state unchecked" instead of calling. Suggest only — never run the archive yourself.
...
- Never edit code, `_index.json`, `PROJECT.md`, `STATUS.md`, or notes.
