---
name: end-session
description: Verify session progress is captured and finalize the session. Use when the user invokes `/end-session`, says "session end" / "end session", or signals they're stopping work for now. Progress notes are written *continuously* during the session via [[resume-session]] — `/end-session` is a verification + commit step, not the moment when context is first dumped.
---
...
**Progress is written as you go, not at the end.** When [[resume-session]] runs (or as soon as a project is established for the conversation), open a `projects/[project]/progress/[date]-slug.md` file and append to it throughout the session:
...
`/end-session` is the **verification + commit** step. The bulk of the writing should already exist by the time it runs.

## Usage

- `/end-session [project]` — end session for a project (optional if set via `/resume-session` this conversation)
...
1. **Locate the in-progress file** for this session: `projects/[project]/progress/[date]-slug.md`. If it does not exist (no `/resume-session` ran, or progress was never opened), create it now with all the context from this conversation.
