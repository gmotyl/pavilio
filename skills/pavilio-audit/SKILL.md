---
name: pavilio-audit
description: Deep, evidence-based repository audit producing an honest health assessment and a prioritized, actionable improvement plan. Runs in four ordered phases (Discovery → Audit → Strategy → Task Plan). Analysis only — never modifies code. Use when the user invokes `/pavilio-audit`, asks to audit a repo, wants a codebase health grade, or needs a prioritized refactor/improvement roadmap.
---
...
Convert the strategy into an execution plan. Break work into discrete tasks. Each task must include:
...
- Task Plan milestones/dependencies → `flowchart LR` or a `gantt` if sequencing matters.

Don't force a chart where prose is clearer — a healthy dimension gets one sentence, not a diagram.
...
- **Repo Map** (include the architecture diagram)
- **Audit Report**
